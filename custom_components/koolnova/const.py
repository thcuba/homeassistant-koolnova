...
MIN_UPDATE_INTERVAL = 5       # configurable minimum
MAX_UPDATE_INTERVAL = 300     # configurable maximum
DEFAULT_PROJECT_UPDATE_FREQUENCY = 10  # frequency for project updates (in cycles)
MIN_PROJECT_UPDATE_FREQUENCY = 1      # configurable minimum (always update)
...
# HVAC Mode mappings for projects - OPTIMIZED: main definition only
...
# Automatically generate reverse mapping for project HVAC
HVAC_TO_KOOLNOVA_MODE = {v: k for k, v in KOOLNOVA_TO_HVAC_MODE.items()}

# Status mappings for sensors/zones - OPTIMIZED: main definition only
...
# Automatically generate reverse mapping for zone status
HVAC_TO_KOOLNOVA_ZONE_STATUS = {v: k for k, v in KOOLNOVA_ZONE_STATUS_TO_HVAC.items()}

# Fan Mode mappings - OPTIMIZED: main definition only
KOOLNOVA_TO_FAN = {
    "1": FAN_LOW,
    "2": FAN_MEDIUM,
    "3": FAN_HIGH,
...
# Automatically generate reverse mapping for fan speed
FAN_TO_KOOLNOVA = {v: k for k, v in KOOLNOVA_TO_FAN.items()}

# Retry constants (non-configurable)
