...
class KoolnovaProjectEntity(ClimateEntity):
    ...
    def preset_mode(self):
        """Return most common zone HVAC mode among project zones."""
        all_sensors = self.coordinator.data.get("sensors", [])
        if not all_sensors:
            return None

        project_id = self._project.get("Topic_id")
        sensors = [s for s in all_sensors if str(s.get("Topic_id")) == str(project_id)]
        if not sensors:
            return None
    ...
    def target_temperature(self):
        """Return median of project zones' target temperatures."""
        all_sensors = self.coordinator.data.get("sensors", [])
        if not all_sensors:
            return None
    ...
    def current_temperature(self):
        """Return average temperature of project zones, rounded to nearest 0.5."""
        all_sensors = self.coordinator.data.get("sensors", [])
        if not all_sensors:
            return None
    ...
    def extra_state_attributes(self):
        ...
        # Get system connectivity data from project's sensors (more up-to-date)
        system_connectivity = {}
        if sensors:
            topic_info = sensors[0].get("topic_info", {})  # Use first sensor from this project
            system_connectivity = {
    ...
    async def async_set_fan_mode(self, fan_mode):
        ...
        try:
            result = await self.coordinator.async_update_all_sensors_fan_speed(speed_code, topic_id=self._project["Topic_id"])
    ...
    async def async_set_preset_mode(self, preset_mode: str):
        ...
        try:
            result = await self.coordinator.async_update_all_sensors_status(status_code, topic_id=self._project["Topic_id"])
            if result.get("failed", 0) > 0:
                async_create(
                    self.hass,
    ...
    async def async_set_temperature(self, **kwargs):
        ...
        try:
            result = await self.coordinator.async_update_all_sensors_temperature(temp, topic_id=self._project["Topic_id"])
            if result.get("failed", 0) > 0:
                async_create(
                    self.hass,
    ...
    def __init__(self, coordinator, config_entry, sensor):
        ...
        if coordinator.data.get("projects"):
            for project in coordinator.data["projects"]:
                if str(project.get("Topic_id")) == str(project_id):
...
class KoolnovaHubEntity(ClimateEntity):
    ...
    def __init__(self, coordinator, config_entry, hub):
        ...
        # Hubs are legacy, but let's try to group them too if they belong to a project
        self._attr_device_info = DeviceInfo(
            identifiers={(DOMAIN, f"{config_entry.entry_id}_hub_{self._hub_id}")},
            name=f"Koolnova Hub {self._hub_id}",
            manufacturer="Koolnova",
            model="Legacy Hub",
        )
        self._attr_should_poll = False
